...
struct PostImagesView: View {
    ...
    /// Every tile gets an explicit height *before* it is clipped, so a photo can
    /// never dictate the card's width.
    var body: some View {
        Group {
            switch urls.count {
            case 0:
                EmptyView()
            case 1:
                imageTile(0, height: 260)
            case 2:
                HStack(spacing: 4) {
                    imageTile(0, height: 200)
                    imageTile(1, height: 200)
                }
            case 3:
                HStack(spacing: 4) {
                    imageTile(0, height: 220)
                    VStack(spacing: 4) {
                        imageTile(1, height: 108)
                        imageTile(2, height: 108)
                    }
                }
            default:
                LazyVGrid(columns: [GridItem(.flexible(), spacing: 4), GridItem(.flexible(), spacing: 4)], spacing: 4) {
                    ForEach(Array(urls.enumerated()), id: \.offset) { index, _ in
                        imageTile(index, height: 130)
                    }
                }
            }
        }
    ...
    private func imageTile(_ index: Int, height: CGFloat) -> some View {
