...
struct MessageBubble: View {
    ...
    private var bubble: some View {
        VStack(alignment: .leading, spacing: 6) {
            ...
            HStack(spacing: 4) {
                // `Spacer` (not `maxWidth: .infinity`) pushes the timestamp to
                // the right *without* stretching the bubble to full width.
                Spacer(minLength: 0)
                if message.isEdited {
                    Text("изменено")
        ...
        HStack(spacing: 4) {
            ForEach(0..<3, id: \.self) { index in
                Circle()
                    ...
                    .scaleEffect(CGFloat(1 + 0.35 * sin(phase + Double(index) * 0.8)))
            }
        }
        .padding(.horizontal, 12)
        .padding(.vertical, 10)
