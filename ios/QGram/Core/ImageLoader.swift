...
struct RemoteImage<Placeholder: View>: View {
    ...
    var body: some View {
        ZStack {
            if let image {
                // The container decides the size, never the picture: a `.fill`
                // image left to size itself pushes past its frame and stretches
                // whatever holds it (post cards did exactly that).
                Color.clear
                    .overlay(
                        Image(uiImage: image)
                            .resizable()
